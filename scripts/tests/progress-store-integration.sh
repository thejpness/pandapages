...
readonly database_password='generated-progress-store-password-not-for-production'
readonly postgres_image=${PP_PROGRESS_STORE_TEST_POSTGRES_IMAGE:-postgres:18.1-alpine}
readonly container_name="pandapages-progress-store-test-$$"
...
docker run --detach \
  --name "$container_name" \
  --label com.pandapages.disposable=progress-store-integration \
...
(
  ...
  PP_PROGRESS_STORE_TEST_DISPOSABLE=1 \
    PP_PROGRESS_STORE_TEST_DATABASE_URL="$database_url" \
    go test ./internal/db -run '^TestProgressStoreIntegration$' -count=1
)

printf 'progress_store_integration=passed\n'
