...
CREATE TABLE story_versions (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  story_id bigint NOT NULL REFERENCES stories(id),
  edition_id bigint NOT NULL REFERENCES story_editions(id),
...
ALTER TABLE stories
  ADD CONSTRAINT stories_current_release_backup_test_fkey
...
INSERT INTO story_versions (story_id, edition_id, markdown)
SELECT story.id, edition.id, body.markdown
FROM stories AS story
JOIN story_editions AS edition ON edition.story_id = story.id
CROSS JOIN (
...
SELECT release.id, story.id, edition.id, version.id
FROM stories AS story
JOIN story_releases AS release ON release.story_id = story.id
JOIN story_editions AS edition ON edition.story_id = story.id
