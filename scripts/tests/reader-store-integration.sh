...
[[ $(query "SELECT count(*) FROM information_schema.tables WHERE table_schema='public' AND table_name IN ('story_editions','story_releases','story_release_editions','story_sources','story_source_versions','story_orchestration_runs','story_orchestration_run_editorial_reviews','story_orchestration_run_draft_ingests','story_orchestration_run_draft_ingest_editions','story_generation_jobs','story_generation_usage_events','source_acquisitions','source_acquisition_eligibility_assessments','source_acquisition_quality_reviews','reader_story_edition_overrides','reading_progress');") == 16 ]]
[[ $(query "SELECT count(*) FROM pg_constraint WHERE conname IN ('story_orchestration_run_editorial_reviews_id_run_key','story_orchestration_run_draft_ingests_review_run_fkey','story_orchestration_run_draft_ingests_editorial_review_key','story_orchestration_run_draft_ingest_editions_ingest_fkey','story_orchestration_run_draft_ingest_editions_version_edition_fkey','story_generation_jobs_source_version_fkey','story_generation_jobs_requester_membership_fkey','story_generation_jobs_completed_run_fkey','story_generation_jobs_status_check','story_generation_jobs_stage_check','story_generation_jobs_lifecycle_check','story_generation_usage_events_job_fkey','story_generation_usage_events_provider_response_key','story_generation_usage_events_operation_check','story_generation_usage_events_identifier_check','story_generation_usage_events_tokens_check');") == 16 ]]
[[ $(query "SELECT count(*) FROM pg_indexes WHERE schemaname='public' AND indexname IN ('story_generation_jobs_one_active_source_idx','story_generation_jobs_source_created_idx','story_generation_usage_events_job_observed_idx');") == 3 ]]
