...
| F44 Database migrations | Operator | Goose one-shot before API | Migrations `00001`–`00012` | Full schema and fixtures | Generated PostgreSQL suite applies all | Functional but fragile | High | Compose migrate service; integration test | Production stream includes explicitly named test seed migration `00008`. |
...
| APP-012 | Normal migration stream installs explicit test fixtures | Migrations | Medium | Operability | migrations/00008_seed_test_data.sql; both Compose run all migrations | Fresh real environment receives sample child/prompt/stories/progress/job | High | Forward-safe fixture/product-data decision; separate dev seeding | PR 7 |
...
9. **Bundled content:** Are the migration 00008 stories/profiles intentional product samples or test fixtures only? Removal from existing environments cannot be guessed safely.
